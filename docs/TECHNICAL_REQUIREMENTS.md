...
| Pixel setup | 4.7 defaults new projects to `canvas_items` stretch mode, so configure pixel rendering explicitly. World in a low-res SubViewport (e.g. 320×180, Nearest filter, `snap_2d_transforms_to_pixel` on, integer scale); UI at native resolution so text stays crisp. **Fixed camera** in v0 (camera smoothing + pixel snap jitter is an unsolved engine issue) |
| World | 16 px grid; TileMapLayers for floor/walls/furniture; `y_sort_enabled`; character origin at the feet |
...
| Characters | Layered Sprite2Ds driven by one AnimationPlayer (keeps layers in sync), palette/ramp-swap shader per material slot, baked into one texture when the outfit changes |
